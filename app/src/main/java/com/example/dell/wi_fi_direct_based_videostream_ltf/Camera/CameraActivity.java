...
//                   testencoder.stop();
        ...
        setBitrate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v){
                try{
                    rateAdaptiveEncoder.adjustBitrate(Integer.parseInt(mBitrate.getText().toString()));
//                    testencoder.changebitrate(Integer.parseInt(mBitrate.getText().toString()));
    ...
    protected void onResume() {
        super.onResume();
        if(!usesoftencoder) { initCameraThread(); }         //changed
    ...
    private SurfaceHolder.Callback mSurfaceHolderCallback = new SurfaceHolder.Callback() {
        ...
        public void surfaceCreated(SurfaceHolder holder) {
            if(usesoftencoder){
                if (mCamera == null) {
                    Log.d(TAG,"camera1我要开始startcamera咯！");
                    startCamera(holder);
                }
            }else{
    ...
    private void startPreView() {
        if(usesoftencoder){
            try {
                mCamera.setPreviewDisplay(mSurfaceHolder); // 先绑定显示的画面
...
//                        testencoder.fireVideo(data,stamp);
                        rateAdaptiveEncoder.encode(data,stamp);
                    }
                }
            });
        }
    ...
    private void startCamera(final SurfaceHolder holder) {
        ...
        if (mCamera != null) {
            ...
            int[] range = findClosestFpsRange(30, parameters.getSupportedPreviewFpsRange());
